...
describe('GameLogic.e2e - Iceblock', () => {
	it('Iceblock should not move initiially', () => {
		const [, level] = SessionPlayer.play(
			TestLevelBuilder.newLevel().addEntity(new Iceblock(new Pushable()), 5, 5),
			[PlayerAction.Wait],
		);

		levelAssert.assertEntityAt(level, EntityType.Iceblock, 5, 5);
	});
	...
	PlayerActionUtils.moves.forEach((action) => {
		it(`Player should push the iceblock into empty floor with move ${PlayerAction[action]} and it should keep moving`, () => {
			...
			const [, level] = SessionPlayer.play(
				TestLevelBuilder.newLevel().addEntity(new Iceblock(new Pushable()), 10 + getX(moveDirection), 10 + getY(moveDirection)),
				[action, PlayerAction.Wait, PlayerAction.Wait],
			);

			levelAssert.assertEntityAt(level, EntityType.Iceblock, 10 + getX(moveDirection) * 4, 10 + getY(moveDirection) * 4);
		});
		...
		it(`No movement when trying to push iceblock ${PlayerAction[action]} into another iceblock`, () => {
			...
			levelAssert.assertEntityAt(level, EntityType.Protagonist, 10, 10);
			levelAssert.assertEntityAt(level, EntityType.Iceblock, iceblock1X, iceblock1Y);
			levelAssert.assertEntityAt(level, EntityType.Iceblock, iceblock2X, iceblock2Y);
		});
		...
		it(`Iceblock stops moving when it moves ${PlayerAction[action]} into a wall`, () => {
			...
			levelAssert.assertEntityAt(level, EntityType.Iceblock, 10 + getX(moveDirection) * 3, 10 + getY(moveDirection) * 3);
	...
	roomEdgeMoves2.forEach(([x, y, action, expectedX, expectedY]) => {
		it(`Iceblock stops moving when it tries to move ${PlayerAction[action]} out of bounds`, () => {
			...
			levelAssert.assertEntityAt(level, EntityType.Iceblock, expectedX, expectedY);
	...
	it('Iceblock should extinguish Fireball and melt, ejecting its contents when it is pushed into it', () => {
		...
		levelAssert.assertEntityCount(level, EntityType.Iceblock, 0);
		levelAssert.assertEntityAt(level, EntityType.Pushable, 10, 8);
	});
	...
	it('Iceblock should extinguish Fireball and melt, ejecting its contents when it moves into it', () => {
		...
		levelAssert.assertEntityCount(level, EntityType.Fireball, 0);
		levelAssert.assertEntityCount(level, EntityType.Iceblock, 0);
		levelAssert.assertEntityAt(level, EntityType.Pushable, 10, 7);
	});
